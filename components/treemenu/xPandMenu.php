...
define('NODE_DEFAULT_IMG','components/treemenu/folderclose.gif');
define('LEAF_DEFAULT_IMG','components/treemenu/doc.gif');
define('NODE_DEFAULT_ALT_IMG','components/treemenu/folderopen.gif');
define('LEAF_DEFAULT_ALT_IMG','components/treemenu/doc.gif');
...
class XMenu
{
	...
	function generateTree($root = false)
	{
		...
		foreach($root->items as $myChild){
			$this->imgCount ++;
			if($myChild->img){
				$img = "<img id=\"Ximg".$this->imgCount."\" src=\"".$myChild->img."\" border=\"0\">\n";
				if($myChild->alt_img){
			...
			if(count($myChild->items) != 0){
				$this->output .= $this->codeIndent()."<li class=\"Xnode\"><div onClick=\"".$img_js."xMenuShowHide(document.getElementById('Xtree".$this->treeCount."'));\">".$href_open.$img.$myChild->name.$href_close."</div></li>\n";
				$this->indent ++;
